...
**Sensitive data in logs and traces.** A short list of fields must never appear in logs or traces: passwords (whether plaintext or hashed), session tokens, API keys, webhook shared secrets, and the signing key. The exclusion is enforced by code review and by an explicit allow-list of fields that the logger formatter is permitted to include. A test in the test suite asserts that a known sensitive field, when passed to any logger context, is redacted in the output. The same allow-list applies to span attributes; a test asserts the same for traces.
...
**Bypass for operator-level access.** The bootstrap CLI subcommand and the data export job need to operate across tenant boundaries or to set up the initial state of a tenant before any user has been created. These code paths run as a different database role that is exempted from RLS policies by virtue of being a Postgres BYPASSRLS role. The bypass role is used only by the bootstrap CLI and by the export job, both of which are operator-level rather than user-level execution paths, and the bypass is granted only to those specific code paths via a different connection pool configured at startup. Application code that runs on behalf of a user authenticated through the dashboard or a reader authenticated through the API key never reaches the bypass pool.

**A verification test.** The test suite contains an explicit test that asserts the two layers work together. The test creates two tenants and two members, one in each tenant. It then opens a connection bound to tenant A and attempts to query the member belonging to tenant B by its identifier. The test asserts that the query returns no rows. The test then deliberately bypasses the application-layer filter by issuing a raw SQL query without the tenant predicate, asserts that the RLS layer still filters the rows. Finally the test bypasses the RLS layer by using the BYPASSRLS role and asserts that both members are visible, confirming that the protection comes from RLS specifically rather than from some other accident. The test is part of the contract test suite and runs against the Postgres adapter implementation.
...
The tenant provisioning and identity components are realized as the `TenantManagement`, `UserManagement`, and `SessionManagement` use cases in the application layer, together with the `casbin` driven adapter and the bootstrap CLI subcommand. The use cases follow the patterns from sections two (event sourcing), three (read model projection), seven (port contracts), and nine (authentication), and the components do not introduce any pattern beyond those.

The tenant aggregate is a small aggregate with events for tenant creation, configuration changes, and tenant suspension or reactivation. The user aggregate has events for user creation, role change, password change, and deactivation. The session aggregate is a degenerate aggregate (a session has exactly one event, its creation, after which it is mutated only by direct row updates of last_seen_at and expiry); sessions are therefore not strictly event-sourced but use direct row updates against the sessions table. The choice to break event-sourcing discipline for sessions is intentional: session activity updates would dominate the event volume if event-sourced, and the audit value of every session refresh event is low.
