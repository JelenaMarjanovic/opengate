...
**Command idempotency.** The dashboard sends an `Idempotency-Key` HTTP header with every state-changing request. The key is a client-generated UUID that the dashboard persists in its request context until the request is acknowledged. On the server side, mutating handlers are wrapped by an idempotency middleware that runs after the session middleware, because it reads both the tenant and the authenticated principal from the request context. The middleware applies only to mutating methods; safe methods pass through untouched. A wrapped mutating request that arrives without the header is rejected with 400 and a Problem Details body, so the contract is enforced rather than silently optional.

The middleware follows an execute-then-record pattern. It buffers the request body up to one mebibyte — a larger body is rejected with 413, because a body the middleware cannot hash is a body it cannot protect — computes a SHA-256 fingerprint over the authenticated actor identifier followed by the body, and looks up the tuple (tenant_id, idempotency_key) in `command_idempotency_keys`. A hit whose fingerprint matches replays the stored response without invoking the handler. A hit whose fingerprint differs is rejected with 409 Conflict, which covers both the same-key-different-payload case and the case of a second caller inside the same tenant reusing a key they obtained; folding the actor identifier into the fingerprint is what makes the second case a refusal rather than a disclosure. A miss runs the handler against a capturing recorder and records the result.

The alternative pattern, reserving the key before executing the handler, was considered and rejected. A reservation row makes the concurrent case cheaper but introduces a failure mode with no bounded recovery: a crash between writing the reservation and updating it with the response leaves a pending row that answers 409 to every retry for the full retention window, including for an operation that in fact completed. Execute-then-record has the opposite trade-off — in a genuine race both requests execute — and that duplication is collapsed by the command handler's own transaction, which performs all of its side effects under optimistic concurrency so that the loser's transaction conflicts and rolls back. The guarantee the middleware provides is therefore that both clients observe one stored response, not that the handler runs once. Single-transaction, optimistically-concurrent command handlers are consequently a binding constraint on every wrapped endpoint rather than a stylistic preference.

Only successful responses are cached. A cached transient 5xx would be replayed to every retry for the full retention window, which is the same unbounded recovery harm the reservation pattern was rejected for, and a 5xx is not evidence that the operation completed. Non-2xx responses are returned uncached, but the middleware still re-reads the key before returning one: if a concurrent request recorded a success under the same key and fingerprint, the client receives that stored success rather than its own error, which is the correct outcome when a handler's transaction lost an optimistic-concurrency race to its own duplicate.

A cached response stores its status, its body, and an allow-list of response headers — content type, location, and entity tag. Storing the headers is what makes a replay equivalent to the original; without them a replayed JSON body is content-sniffed and delivered as plain text. The list is an allow-list rather than a deny-list so that the failure mode of omitting a header is a missing header rather than a leaked one; `Set-Cookie` in particular is never stored, because a session cookie minted for one request and replayed to another minutes later is a session-handling defect. A response larger than one mebibyte is delivered to the client but not cached.

Two paths deliberately continue without protection rather than failing the request: a failed record, and a failed re-read after the handler has run. Both are correct, since a handler that has already executed cannot be un-executed, and both are counted and logged, because a silent fail-open is indistinguishable from a working cache. A failed lookup before the handler runs is the opposite case and fails closed with a 500, since the middleware cannot distinguish a retry from a first attempt; it needs no counter because it is already visible as a server error.

The retention window for command idempotency keys is ten minutes, informed by the dashboard's longest reasonable retry budget and by the unbounded storage growth of cached responses at longer windows. A cleanup job runs every five minutes on a dedicated maintenance queue, takes a deployment-wide advisory lock so that a tick on a multi-instance deployment is skipped rather than duplicated, and deletes rows older than the window from the command and decision tables. The boundary is evaluated by the database rather than by the worker, so it cannot drift with clock skew between worker instances. Because Postgres requires SELECT privilege on any column a statement reads to evaluate its qualifier, the job's role holds a column-scoped SELECT on `created_at` in addition to DELETE — column-scoped rather than table-wide, so the cleanup can find expired rows without gaining read access to cached response bodies.

Because a client cannot reason about retry safety without knowing how long a key survives, the retention window is part of the API contract and is stated in the API documentation.

**Decision idempotency.** Reader-originated access requests also carry an `Idempotency-Key` header, generated by the reader when it initiates the request. The server-side mechanism is not the command middleware. The decision cache entry is written inside the AccessDecision use case's own transaction, alongside the access-attempted event append, as specified in section sixteen: on the decision path the idempotency record and the event append must be atomic with each other, and an HTTP middleware that records after the handler returns cannot provide that. The table is separate (`decision_idempotency_keys`) so that the two classes of caller do not contend on one hot table and so that the decision table's indexes are tuned to its lookup pattern, and its retention window is also ten minutes, enforced by the same cleanup job. The decision table stores the decision and its reason code rather than a request fingerprint, because the decision path's telemetry needs the outcome and the request shape is fixed by the reader protocol.
...
**Implementation of the idempotency lookup.** The command middleware performs an upfront `SELECT` on the tuple and, on a miss, runs the handler and then issues an `INSERT` with an `ON CONFLICT DO NOTHING` clause, re-reading the stored row if the insert found a conflict. The upfront read is what allows a retry arriving after the first request completed to skip the handler entirely, which is the point of the mechanism; an insert-first formulation cannot do that without also writing a reservation row, which is the rejected pattern. The cost is one read on the replay path and one read plus one write on the miss path. An implementation using Postgres advisory locks per idempotency key was considered and rejected as more expensive without sufficient benefit.
...
**Job kinds.** Job kinds are Go types that implement the `river.JobArgs` interface and have a `Kind` method returning a stable string identifier. The job kinds in OpenGate are enumerated as follows. The `projector.audit_log` kind runs the audit log read model projector. The `projector.door_status` kind runs the door status read model projector. The `projector.subscription_delivery` kind runs the subscription delivery (dead-letter) read model projector. The `projector.export_status` kind runs the export status read model projector. The `projector.reader_connectivity` kind runs the reader connectivity read model projector. The `subscription.deliver` kind delivers a single webhook attempt. The `export.run` kind executes a tenant data export job. The `export.audit_csv` kind streams audit query results into a CSV file for download. The `cleanup.idempotency_keys` kind purges expired idempotency keys. The `cleanup.sessions` kind purges expired sessions. The `cleanup.dead_letter` kind purges dead-letter queue entries past their retention window. The `reader.notify` kind fans out a credential or policy update to active SSE streams; this kind is used in cases where the synchronous LISTEN/NOTIFY path is not appropriate, such as when the notification originates from a projector rather than from a command handler.

**Where a retention job lives.** A retention job is delivered by the story that creates the data it prunes — not by a separate cleanup story, and not by the story that happens to build the queue. Retention is part of the definition of done for any table that grows without bound, in the same way that a table's migration belongs to the story whose code reads it. A collecting cleanup story that gathers jobs across epics is explicitly rejected: it accumulates with every epic, it is nobody's definition of done, and it is how the `cleanup.sessions` job was deferred out of US-02.03 in the v1.1 reconciliation without a destination and then went unnoticed until Sprint 5.
