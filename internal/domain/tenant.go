...
// NewTenant builds an active Tenant with the given ID and name, applying the
// DB §5.1 defaults (UTC timezone, 60-minute session timeout, active status).
// The name is trimmed and must be non-empty. The ID is passed in so the
// constructor stays pure (no clock, no randomness); the caller mints the UUID.
